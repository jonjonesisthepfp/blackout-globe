import { J as BufferGeometry, bn as Float32BufferAttribute } from "./three.module-BLr34Qqw.js";
...
function flatten(data) {
	...
	for (const ring of data) {
		for (const p of ring) for (let d = 0; d < dimensions; d++) vertices.push(p[d]);
		if (prevLen) {
			holeIndex += prevLen;
...
//#endregion
//#region node_modules/three-geojson-geometry/dist/three-geojson-geometry.mjs
...
var GeoJsonGeometry = /* @__PURE__ */ function(_THREE$BufferGeometry) {
	function GeoJsonGeometry(geoJson) {
		...
		function genPoint(coords, r) {
			return [{
				vertices: polar2Cartesian(coords[1], coords[0], r + (coords[2] || 0)),
				indices: []
			}];
		}
		function genMultiPoint(coords, r) {
			...
			coords.map(function(c) {
				...
				var newPnt = _slicedToArray(_ref, 1)[0];
				concatGroup(result, newPnt);
			});
			return [result];
		}
		function genLineString(coords, r) {
			var vertices = flatten([interpolateLine(coords, resolution).map(function(_ref3) {
				var _ref4 = _slicedToArray(_ref3, 3), lng = _ref4[0], lat = _ref4[1], _ref4$ = _ref4[2];
				return polar2Cartesian(lat, lng, r + (_ref4$ === void 0 ? 0 : _ref4$));
			})]).vertices;
			var numPoints = Math.round(vertices.length / 3);
			var indices = [];
			for (var vIdx = 1; vIdx < numPoints; vIdx++) indices.push(vIdx - 1, vIdx);
			return [{
				vertices,
				indices
			}];
		...
		function genMultiLineString(coords, r) {
			...
			coords.map(function(c) {
				...
				var newLine = _slicedToArray(_ref5, 1)[0];
				concatGroup(result, newLine);
			});
			return [result];
		}
		function genPolygon(coords, r) {
			var _earcutFlatten2 = flatten(coords.map(function(coordsSegment) {
				return interpolateLine(coordsSegment, resolution).map(function(_ref7) {
					var _ref8 = _slicedToArray(_ref7, 3), lng = _ref8[0], lat = _ref8[1], _ref8$ = _ref8[2];
					return polar2Cartesian(lat, lng, r + (_ref8$ === void 0 ? 0 : _ref8$));
				});
			})), vertices = _earcutFlatten2.vertices, holes = _earcutFlatten2.holes;
			...
			for (var vIdx = 1; vIdx < numPoints; vIdx++) if (!holesIdx.has(vIdx)) if (vIdx < firstHoleIdx) outerIndices.push(vIdx - 1, vIdx);
			else holeIndices.push(vIdx - 1 - firstHoleIdx, vIdx - firstHoleIdx);
			var groups = [{
				indices: outerIndices,
				vertices: outerVertices
			}];
			if (holes.length) groups.push({
				indices: holeIndices,
